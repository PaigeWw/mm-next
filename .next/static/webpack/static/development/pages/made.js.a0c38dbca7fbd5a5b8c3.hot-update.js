  ...
  Object(react__WEBPACK_IMPORTED_MODULE_0__["useEffect"])(function () {
    ...
    for (var i = 0; i < svgElement.children.length; i++) {
      if (svgElement.children[i].tagName === "g" || svgElement.children[i].tagName === "path") {
        svgElement.children[i].style.fill = colors[i].type ? "url(\"#".concat(svgId, "-").concat(colors[i]._id, "-").concat(i, "\")") : colors[i].value;
      }
    }
  }, [colors]);
  return __jsx("div", {
      ...
      if (colors && colors.length > 0 && colors[0]) {
        ...
        for (var i = 0; i < svg.children.length; i++) {
          if (svg.children[i].tagName === "g" || svg.children[i].tagName === "path") {
            console.log(colors);
            svg.children[i].style.fill = colors[_j].type ? "url(\"#".concat(svgId, "-").concat(colors[_j]._id, "-").concat(_j, "\")") : colors[_j].value;
            _j++;
          }
        }
      }
    ...
    beforeInjection: function beforeInjection(svg) {
      ...
      for (var i = 0; i < colors.length; i++) {
        ...
        if (color.type) {
          ...
          svgPattern.setAttribute("id", "".concat(svgId, "-").concat(colors[j]._id, "-").concat(j)); // editPatterns[color._id] = svgPattern
...
//# sourceMappingURL=made.js.a0c38dbca7fbd5a5b8c3.hot-update.js.map