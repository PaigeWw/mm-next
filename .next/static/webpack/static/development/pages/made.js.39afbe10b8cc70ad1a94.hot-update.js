      ...
      if (colors && colors.length > 0 && colors[0]) {
        ...
        for (var i = 0; i < svg.children.length; i++) {
          if (svg.children[i].tagName === "g" || svg.children[i].tagName === "path") {
            console.log(colors);
            svg.children[i].style.fill = colors[j].type ? "url(\"#".concat(id, "-").concat(colors[j]._id, "-").concat(j, "\")") : colors[j].value;
            j++;
          }
        }
      }
...
//# sourceMappingURL=made.js.39afbe10b8cc70ad1a94.hot-update.js.map