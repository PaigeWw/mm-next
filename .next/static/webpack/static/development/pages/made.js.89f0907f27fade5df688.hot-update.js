      ...
      if (colors && colors.length > 0 && colors[0]) {
        ...
        for (var i = 0; i < svg.children.length; i++) {
          if (svg.children[i].tagName === "g" || svg.children[i].tagName === "path") {
            console.log(colors);
            svg.children[i].style.fill = colors[j].type ? "url(\"#".concat(svgId, "-").concat(colors[j]._id, "-").concat(j, "\")") : colors[j].value;
            j++;
          }
        }
      }
    ...
    beforeInjection: function beforeInjection(svg) {
      ...
      for (var i = 0; i < colors.length; i++) {
        ...
        if (color.type) {
          ...
          svgPattern.setAttribute("id", "".concat(svgId, "-").concat(color._id, "-").concat(i)); // editPatterns[color._id] = svgPattern
...
//# sourceMappingURL=made.js.89f0907f27fade5df688.hot-update.js.map